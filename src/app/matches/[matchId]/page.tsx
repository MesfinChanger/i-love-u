...
import { Send, ChevronLeft, Sparkles, Loader2, Camera, ShieldAlert, EyeOff } from 'lucide-react';
...
export default function ChatPage() {
  ...
  // Mock match details (In real app, we would fetch from /users/{matchId})
  ...
  const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      reader.onloadend = async () => {
        ...
        addDoc(collection(db, 'matches', String(matchId), 'messages'), {
          senderId: user.uid,
          imageUrl: dataUri, // In a real app, upload to Storage and use URL
