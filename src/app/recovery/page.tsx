    ...
    <div className="flex flex-col min-h-screen bg-muted/30 pb-24 items-center justify-center p-6">
      <div className="w-full max-w-md space-y-8 animate-in fade-in duration-700">
        <div className="text-center space-y-4">
          ...
          <p className="text-muted-foreground font-medium italic">"Restore your connection securely."</p>
        </div>
        ...
        <Card className="border-none shadow-2xl rounded-[2.5rem] overflow-hidden bg-white">
          ...
          <CardContent className="p-8 space-y-4">
            <RecoveryLink href="/forgot-password" icon={<KeyRound className="w-5 h-5" />} title="Forgot Phrase" desc="Reset access via email." />
            <RecoveryLink href="/forgot-username" icon={<User className="w-5 h-5" />} title="Forgot Username" desc="Retrieve community signature." />
            <RecoveryLink href="/identity" icon={<Smartphone className="w-5 h-5" />} title="Lost Device" desc="Migrate identity safely." />
          </CardContent>
        </Card>
        ...
        <div className="flex flex-col items-center gap-4">
           <Link href="/login" className="flex items-center justify-center gap-2 text-slate-400 hover:text-primary font-bold text-[10px] uppercase tracking-widest">
              <ArrowLeft className="w-3.5 h-3.5" /> Back to Identify
           </Link>
        </div>
      </div>
    </div>
  ...
  return (
    <Link href={href} className="group block">
      <div className="p-5 rounded-2xl border-2 border-transparent hover:border-primary/10 hover:bg-primary/5 transition-all flex items-center gap-5">
        <div className="w-12 h-12 rounded-xl bg-muted flex items-center justify-center text-slate-400 group-hover:bg-white group-hover:text-primary transition-all">
          {icon}
        </div>
        <div>
