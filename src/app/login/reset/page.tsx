  ...
  const handleResetRequest = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast({
        variant: "destructive",
        title: "Access Ripple",
        description: "We couldn't find a heart associated with that email.",
      });
    } finally {
      setIsLoading(false);
    }
              ...
              <div className="text-center space-y-6 animate-in fade-in zoom-in-95 duration-500">
                ...
                <div className="space-y-2">
                  ...
                  <p className="text-xs text-muted-foreground font-medium italic leading-relaxed">
                    ...
                    Please follow the instructions to reset your secure phrase.
                  </p>
                </div>
                <Button asChild className="w-full h-16 rounded-2xl gradient-bg font-black uppercase tracking-widest text-[10px] shadow-lg">
              ...
              <form onSubmit={handleResetRequest} className="space-y-8">
                <div className="space-y-4">
                  ...
                  <p className="text-[9px] text-muted-foreground italic font-medium">
                    "Every heart is valued." Enter the email you used to join the movement.
                  </p>
                </div>

                <Button 
                  type="submit"
                  disabled={isLoading || !email || !auth} 
                  className={cn(
                    "w-full h-20 rounded-[2rem] font-black uppercase tracking-[0.3em] text-sm shadow-xl active:scale-95 transition-all",
                    (!email || !auth) ? "bg-slate-200 text-slate-400" : "gradient-bg"
                  )}
                >
